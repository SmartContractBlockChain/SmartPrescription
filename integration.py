...
def deploy_contract(w3, contract_interface, from_account, medicineName, patientAddress):
    tx_hash = w3.eth.contract(
        abi=contract_interface['abi'], bytecode=contract_interface['bin']).constructor(medicineName,
                                                                                       patientAddress).transact(
        {'from': from_account})
...
if __name__ == '__main__':
    ...
    address = deploy_contract(w3, contract_interface, w3.eth.accounts[0], 'viagra', w3.eth.accounts[0])
